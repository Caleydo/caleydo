...
public class GLVisBricks extends AGLView implements IDataContainerBasedView,
	...
	gl.glColor4f(ARCH_COLOR[0], ARCH_COLOR[1], ARCH_COLOR[2],
		ARCH_COLOR[3] * 2);
	...
	if (recordIDCategory == null) {
	    ATableBasedDataDomain dataDomain = newDataContainers.get(0)
		    .getDataDomain();
	...
	for (DataContainer data : newDataContainers) {
	    if (!data.getDataDomain().getRecordIDCategory()
		...
		if (data instanceof PathwayDimensionGroupData) {
		    dimensionGroup
			    .setBrickConfigurer(new PathwayDataConfigurer());
		    // dimensionGroup
		    ...
		    dimensionGroup
			    .setBrickConfigurer(new NumericalDataConfigurer(
				    data));
		    dimensionGroup
			    .setBrickSortingStrategy(new AverageValueSortingStrategy());
		}
	...
	recordSelectionManager = new RecordSelectionManager(
		IDMappingManagerRegistry.get().getIDMappingManager(
