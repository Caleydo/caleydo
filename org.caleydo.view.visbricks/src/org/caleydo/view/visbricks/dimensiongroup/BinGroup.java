...
public class BinGroup extends DimensionGroup {
	...
	ContentVirtualArray contentVirtualArray;
	...
	public void initialize() {
		...
		HashMap<String, ArrayList<Integer>> bin = binning.getBinning(contentIDType,
				binnigIDType, contentVA);

		contentVirtualArray = new ContentVirtualArray();
		ContentGroupList groupList = new ContentGroupList();

		for (ArrayList<Integer> group : bin.values()) {
			for (Integer id : group)
				contentVirtualArray.append(id);

			groupList.append(new Group(group.size()));

		}
		contentVirtualArray.setGroupList(groupList);

//		replaceContentVA(set.getID(), set.getDataDomain().getDataDomainType(),
//				Set.CONTENT);
