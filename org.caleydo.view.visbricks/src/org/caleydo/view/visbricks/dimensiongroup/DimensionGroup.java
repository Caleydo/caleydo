...
public class DimensionGroup extends ATableBasedView implements
     ...
     */
    public void setBrickSortingStrategy(
	    IBrickSortingStrategy brickSortingStrategy) {
	...
	for (DataContainer brickData : brickDataContainers) {
	    GLBrick segmentBrick = createBrick(new ElementLayout("brick"),
		    brickData);
	...
	if (!dataContainer.getRecordPerspective().getID()
	...
	detailBrick.setBrickLayoutTemplate(
		new DetailBrickLayoutTemplate(detailBrick, this, visBricks,
			detailBrick.getBrickConfigurer()), brick
			.getCurrentViewType());
	...
	if (otherDimensionGroup != null) {
	    otherDimensionGroupShowsDetail = otherDimensionGroup
		    .isDetailBrickShown();
