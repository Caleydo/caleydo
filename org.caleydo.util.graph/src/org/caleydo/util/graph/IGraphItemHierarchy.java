...
public interface IGraphItemHierarchy
{
	 ...
	 * 
	 * @param type define type; if prop == EGraphItemHierarchy.NONE or null all
	 *            types are matched.
	 * @return List of IGraph matching type; if no match is found an empty List
	 *         is returned, null is never retrunred.
	 */
	public List<IGraph> getAllGraphByType(EGraphItemHierarchy type);

	/**
	 * Note, if type == EGraphItemHierarchy.GRAPH_NONE or null all types are
	 * matched.
	 * 
	 * @param graph graph to be added
	 * @param type define type; if type == EGraphItemHierarchy.GRAPH_NONE or
	 *            null all types are matched
	 */
	public void addGraph(IGraph graph, EGraphItemHierarchy type);

	/**
	 * Remove IGraph graph. Note, if type == EGraphItemHierarchy.GRAPH_NONE or
	 * null all types are matched.
	 * 
	...
	/**
	 * Test if IGraph graph is contained. Note, if type ==
	 * EGraphItemHierarchy.GRAPH_NONE or null all types are matched.
	 * 
	...
	/**
	 * Test if references to other graphs that to IGraph graph exist ignoring
	 * the EGraphItemHierarchy type. Note this method is intended for
	 * IGraphItem#disposeItem().
	 * 
	 * @see org.caleydo.util.graph.IGraphItem#containsGraph(IGraph,
	 *      EGraphItemHierarchy)
	 * @see org.caleydo.util.graph.IGraphItem#disposeItem()
	 * @param graph test if only this graph is referenced by this IGraphItem
	 *            object
	 * @return FALSE if no other graphs are linked from this IGraphItem, TURE if
	 *         other graphs than the tested graph are linked
	 */
