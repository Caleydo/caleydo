...
public class HTLayouter
	...
	// public HTLayouter(IViewFrustum frustum, PickingManager pickingManager, int
	// iViewID,HyperbolicRenderStyle renderStyle, String strInformation) {
	// super(frustum, pickingManager, iViewID, renderStyle, new DefaultProjection(1), strInformation);//new
	// HyperbolicGlobeProjection(1));
	...
	public HTLayouter(IViewFrustum frustum, PickingManager pickingManager, int iViewID,
		...
		super(frustum, pickingManager, iViewID, renderStyle, new HyperbolicGlobeProjection(1), strInformation);// new
																												// HyperbolicGlobeProjection(1));
	...
	protected void renderTreeLayout() {
		...
		// drawClass.refreshScreenCoordinates(fViewSpaceXAbs, fViewSpaceYAbs, fViewRadius/2.0f,
		// fViewSpaceYAbs/2.0f);
		// drawClass.refreshScreenCoordinates(fViewSpaceXAbs/3, fViewSpaceYAbs/3, fWidth / 2.0f, fHeight /
		// 2.0f);
		// Set<IDrawAbleNode> set = drawClass.drawNodes();
		// for(IDrawAbleNode node : set)
		{
			// drawClass.refreshScreenCoordinates(fViewSpaceXAbs/4, fViewSpaceYAbs/4, fWidth / 2.0f, fHeight /
			// 2.0f);
	...
	private void runThroughTreeAndPlace(IDrawAbleNode node, int layer) {
		...
		if (layer < HyperbolicRenderStyle.MAX_DEPTH) {
			...
			// placeNode(node, (fWidth/2 + node.getXCoord())*fScalingFactor, (fHeight/2 +
			// node.getYCoord())*fScalingFactor, 0.0f, 0.1f, 0.1f);
			// if(node.equals(tree.getRoot()))
			// placeNode(node, node.getRealCoordinates().x(), node.getRealCoordinates().y(), 0.0f, 0.1f,
			// 0.1f);
			// else
			placeNode(node, fWidth / 2 + (node.getRealCoordinates().x() * fScalingFactor), fHeight / 2
				+ (node.getRealCoordinates().y() * fScalingFactor), 0.0f, 0.1f, 0.1f);
			System.out.println(node.getNodeName());
			System.out.println(String.valueOf(node.getProjectedCoordinates().x()) + ' '
				+ String.valueOf(node.getProjectedCoordinates().y()) + ' '
				+ String.valueOf(node.getProjectedCoordinates().z()));
	...
	private float runThroughTreeAndCalculateScalingFactor(IDrawAbleNode node, int layer) {
		...
		if (layer < HyperbolicRenderStyle.MAX_DEPTH) {
			...
			// placeNode(node, fWidth/2 + node.getRealCoordinates().x(), fHeight/2 +
			// node.getRealCoordinates().y(), 0.0f, 0.1f, 0.1f);
			if (tree.getParent(node) != null)
				placeConnection(tree.getParent(node), node);
			...
			{
				originalLineLenth =
					(float) Math.sqrt(Math.pow((tree.getRoot().getXCoord() - node.getRealCoordinates().x()),
						2)
				...
				if (originalLineLenth > fOriginalLineLenth) {
					...
					float fNewScalingFactor =
						treeProjector.getProjectedLineFromCenterToBorder() / fOriginalLineLenth;
