...
/**
 * Helper class that extracts multiple accession numbers mapped to the same storage index. 
 * separated.
 * EXAMPLE: 
 * Input   NM_012156.2;NM_177996.1	1770
 * Output: NM_012156.2;1770
 *         NM_177996.1;1770
 * 
 * @author Marc Streit
 *
...
public class MultipleAccessionCodeExtractor {
	...
	throws IOException {
	    ...
	    BufferedReader in = new BufferedReader(
	      new FileReader("data/genome/mapping/accession_code_2_microarray_expression_storage_index_ORIG.map"));
	    ...
	    {
	    	sAccessionCodes = sInputLine.substring(0, sInputLine.indexOf(cAccessionToExpressionStorageIndexDelimiter));
	    	
			StringTokenizer strTokenText = 
				new StringTokenizer(sAccessionCodes, sMultipleAccessionSeparator);
			...
			{
				sMicroarrayExpressionStorageIndex = sInputLine.substring(
						sInputLine.indexOf(cAccessionToExpressionStorageIndexDelimiter)+1, sInputLine.length());
				
				while (strTokenText.hasMoreTokens()) 
				{
					writer.println(
						strTokenText.nextToken() + cAccessionToExpressionStorageIndexDelimiter + sMicroarrayExpressionStorageIndex);
				}	
			}
	    }
    ...
    public static void main(String[] args) {
    	...
    	try {	
    		MultipleAccessionCodeExtractor enzymeCodeConverter = 
        		new MultipleAccessionCodeExtractor();
