...
/**
 ...
 @deprecated use SingeltonJoglFrameManager instead
...
public class SingeltonFrameManager {
	...
	public static void main(String[] args) {
		...
		new SingeltonFrameManager().run(args);
	}

	protected SingeltonFrameManager() {
		createNewUniqueId();
		
		initDatastructures();	
		
		assert false : "deprecated object!";
	}
	...
	//private void registerInternalFrame( final JInternalFrame refJInternalFrame ) {
		vec_JMenu.put( new Integer(createNewUniqueId()), refJMenu );
	}
	...
	private Container addWindow_AWTcanvas(FrameBaseType which, final boolean bIsInternalFrame) {
		...
		if ( bIsInternalFrame ) {
			...
			inner = createNewJInternalFrame(which.getFrameMenuTitle());
...
//			this.registerJFrame( outer, which.getFrameMenuTitle() );
	...
	private Container addWindow_GLcanvas(FrameBaseType which, final boolean bIsInternalFrame) {
		...
		if ( bIsInternalFrame ) {
			...
			inner = createNewJInternalFrame(str);
...
//		inner2.setVisible(true);
	...
	public synchronized SwingJoglJInternalFrame createNewJInternalFrame( final String title) {
		
		SwingJoglJInternalFrame inner = 
			vec_JFrame.get( vec_JFrameIndex.get(iCurrentFrameIndex).intValue()
					).createJInternalFrame(title);		
		vec_JInternalFrame.put( new Integer(this.createNewUniqueId()), inner );
	...
	public synchronized SwingJoglJFrame createNewJFrame( final String title) {
		...
		SwingJoglJFrame frame = new SwingJoglJFrame( title );	
		int iNewUniqueId = this.createNewUniqueId();
