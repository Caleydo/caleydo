...
public class SelectionSliderBrowser {

	public static void main(String[] args) {

		JFrame baseFrame = new JFrame();

		SelectionSliderBrowser demo = new SelectionSliderBrowser(baseFrame);
	...
	private JSlider js_Offset;
	private JSlider js_Length;
	
	private float fBufferLength = -1;
	...
	public SelectionSliderBrowser(JFrame setRefParentExternalFrame) {
	...
	private void updateOffset( final int iOffset ) {

		if ( refSelection.getWriteToken() ) {
			
			if ( fBufferLength < 0) {
				fBufferLength = (float) refSelection.length() / 200.0f;
			}
			
			refSelection.setOffset( iOffset * 10 );
			
			int iNewCacheId = refSelection.getCacheId() + 2;
			refSelection.setCacheId( iNewCacheId );	
						
			refSelection.returnWriteToken();
			
			jtf_CacheId.setText( Integer.toString(iNewCacheId) );
		}
	}
	
	
	private void updateLength( final int iLength ) {
		...
		if ( refSelection.getWriteToken() ) {
			...
			refSelection.setLength( (int) (fBufferLength * (200-iLength)) );
...
//			refSelection.setId( Integer.valueOf( jtf_Id.getText() ) );	
	...
	public SelectionSliderBrowser(JInternalFrame setRefParentInternalFrame) {
		...
		js_Offset = new JSlider(SwingConstants.HORIZONTAL, 0,200, 10);
		js_Offset.setPaintTicks(true);
		js_Offset.setMajorTickSpacing( 10 );
		js_Offset.addChangeListener( 
				new ChangeListener() 
				{
					public void stateChanged( ChangeEvent e) {						
						updateOffset( js_Offset.getValue() );
					}
				}
		);
		
		js_Length = new JSlider(SwingConstants.HORIZONTAL, 0,200, 200);
		js_Length.setPaintTicks(true);
		js_Length.setMajorTickSpacing( 200 );
		js_Length.addChangeListener( 
				new ChangeListener() 
				{
					public void stateChanged( ChangeEvent e) {
						updateLength( js_Length.getValue() );
					}
				}
		);
		...
		refParentInternalFrame.add( js_Offset, BorderLayout.SOUTH);
		refParentInternalFrame.add( js_Length, BorderLayout.NORTH);
