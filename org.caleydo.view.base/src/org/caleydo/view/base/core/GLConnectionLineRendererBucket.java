package org.caleydo.view.base.core;
...
public class GLConnectionLineRendererBucket extends AGLConnectionLineRenderer {
		...
		for (Entry<EIDType, ConnectionMap> typeConnections : connectedElementRepManager
			...
			for (Entry<Integer, SelectedElementRepList> connections : typeConnections
				...
				for (SelectedElementRep selectedElementRep : connections
					...
					RemoteLevel activeLevel = remoteLevelElement
							.getRemoteLevel();
	...
	private ArrayList<Vec3f> createControlPoints(Vec3f vecSrcPoint,
		...
		controlPoints.add(calculateBundlingPoint(vecSrcPoint,
				vecViewCenterPoint));
	...
	private void renderPlanes(final GL gl, final Vec3f vecPoint,
		...
		for (Vec3f vecCurrent : alPoints) {
			...
			gl.glVertex3f(vecCurrent.x(), vecCurrent.y(),
					vecCurrent.z() - 0.001f);
		}
