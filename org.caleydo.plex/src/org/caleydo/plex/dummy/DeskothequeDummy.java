...
public class DeskothequeDummy {
	...
	public void run() {
		...
		// Ice.ObjectPrx proxy = communicator.stringToProxy(serverInfo.serverName + ":" + serverInfo.endPoint);
		ObjectAdapter adapter = communicator.createObjectAdapterWithEndpoints("desko_dummy", serverInfo.endPoint);
		
		ServerApplication serverApplication = new ServerApplication();
		serverApplication.setCommunicator(communicator);
		serverApplication.setAdapter(adapter);

		adapter.add(serverApplication, communicator.stringToIdentity(serverInfo.serverName));
		adapter.activate();
