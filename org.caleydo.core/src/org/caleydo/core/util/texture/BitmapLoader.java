 ...
 * @author Pepijn Van Eeckhoudt Imported from nehe demos
 */
public class BitmapLoader
{
	...
	public static BufferedImage loadBitmap(String file) throws IOException
	{
		...
		try
		{
			...
			{
				image = read8BitBitmap(nColoursUsed, nBitCount, nSizeImage, nWidth, nHeight,
						input);
			}
			else
			{
				System.out
						.println("BitmapLoader.loadBitmap() Not a 24-bit or 8-bit Windows Bitmap, aborting...");
				image = null;
			}
		}
		finally
	...
	private static BufferedImage read8BitBitmap(int nColoursUsed, int nBitCount,
			int nSizeImage, int nWidth, int nHeight, InputStream input) throws IOException
	{
		...
		for (int n = 0; n < nNumColors; n++)
		{
			npalette[n] = (255 & 0xff) << 24 | (bpalette[nindex8 + 2] & 0xff) << 16
					| (bpalette[nindex8 + 1] & 0xff) << 8 | (bpalette[nindex8 + 0] & 0xff);

			nindex8 += 4;
		}

		int npad8 = (nSizeImage / nHeight) - nWidth;
		BufferedImage bufferedImage = new BufferedImage(nWidth, nHeight,
				BufferedImage.TYPE_INT_ARGB);
		DataBufferInt dataBufferByte = ((DataBufferInt) bufferedImage.getRaster()
				.getDataBuffer());
	...
	private static BufferedImage read24BitBitmap(int nSizeImage, int nHeight, int nWidth,
			InputStream input) throws IOException
	{
		...
		DataBufferByte dataBufferByte = ((DataBufferByte) bufferedImage.getRaster()
				.getDataBuffer());
