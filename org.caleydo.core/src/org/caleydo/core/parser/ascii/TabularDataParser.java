...
public class TabularDataParser extends ATextParser {
	...
	public final boolean setTokenPattern(final String tokenPattern) {
		...
		while (tokenizer.hasMoreTokens()) {
			...
			if (buffer.equalsIgnoreCase("abort")) {
				...
				Logger.log(new Status(IStatus.WARNING, GeneralManager.PLUGIN_ID,
						"Unknown column data type: " + buffer + " in " + tokenPattern));
			}
		}
	...
	public void setTargetColumns(final ArrayList<Integer> targetDimensionIDs) {
		for (int dimensionID : targetDimensionIDs) {
			targetDimensions.add(GeneralManager.get().getColumnManager()
					.getItem(dimensionID));
		}
	}
	...
	@Override
	protected void loadDataParseFile(BufferedReader bufferReader, final int numberOfLines)
			throws IOException {
		...
		while ((line = bufferReader.readLine()) != null
			...
			line = line.replace(tokenSeperator + tokenSeperator, tokenSeperator + "NaN"
					+ tokenSeperator);

			// Take care of empty cells in first column because they are not
			// embedded between two token
			// separators
			...
			if (lineInFile % 1000 == 0) {
				swtGuiManager
						.setProgressBarPercentage((int) (fProgressBarFactor * lineInFile));
			}
		}
	}
