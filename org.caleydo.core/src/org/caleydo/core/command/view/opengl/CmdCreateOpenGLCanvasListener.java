...
extends	ACmdCreate_IdTargetLabelParent {
	 ...
	 * Constructor.
	 */
	public CmdCreateOpenGLCanvasListener(
	...
	/**
	 * Extract set and selection IDs from detail string.
	 * Example: "SET_ID_1 SET_ID_2@SELECTION_ID_1 SELECTION_ID_2"
	 */
	private void extractDataIDs()
	{
		// Read Set and Selection IDs
		StringTokenizer divideSetAndSelectionIDs = new StringTokenizer(
				sDetail,
		...
		{
			StringTokenizer divideIDs = new StringTokenizer(divideSetAndSelectionIDs.nextToken(), 
					IGeneralManager.sDelimiter_Parser_DataItems);
			
			while (divideIDs.hasMoreTokens())
			{
				iArSetIDs.add(StringConversionTool.convertStringToInt(
						divideIDs.nextToken(), -1));
			}
		}
		...
		if(divideSetAndSelectionIDs.hasMoreTokens())
		{
			...
			while (divideIDs.hasMoreTokens())
			{
				iArSelectionIDs.add(StringConversionTool.convertStringToInt(
						divideIDs.nextToken(), -1));
			}
		}	
	}
