...
implements IMediatorReceiver, IMediatorSender, IGLCanvasHeatmap2D {
	...
	private Vec3fGL color_Selection_ConnectionLine_inFocus = new Vec3fGL(0.2f,
			1, 110 / 255);

	private Vec3fGL color_Selection_ConnectionLine_outOffFocus = new Vec3fGL(
			0.1f, 1, 1);

	private Vec3fGL color_Selection_DepthIndicator_fillColor = new Vec3fGL(0,
			0, 0);
	 ...
	 * 
	 * @see GLCanvasHeatmap2DColumn#setSelectedIndex_asArrayList(ArrayList,
	 *      ArrayList)
	 */
	...
	public GLCanvasHeatmap2DColumn(final IGeneralManager generalManager,
		...
		alHighlightSelectionIndex_sortByDepth = new ArrayList<ArrayList<Integer>>(
				5);
...
//				LoggerType.STATUS);
	...
	private int addhighlightSelectionIndex_addItem(
			ArrayList<ArrayList<Integer>> container, final Integer value,
			final int mode, final int containerSize) {
	...
	private void removehighlightSelectionIndex_Item(
			ArrayList<ArrayList<Integer>> container, final Integer value) {
	...
	protected void assignHighlightSelectionIndex(
		...
		while (iterIndex.hasNext())
		{
			...
			if (bufferMode > 0)
			{
				iSizeContainer = addhighlightSelectionIndex_addItem(
						this.alHighlightSelectionIndex_sortByDepth, buffer,
						bufferMode, iSizeContainer);

				hashHighlightSelectionIndex.put(buffer, new Boolean(true));
			} else
			{
				removehighlightSelectionIndex_Item(
						alHighlightSelectionIndex_sortByDepth, buffer);
	...
	public void setSelectedIndex_asArrayList(
		...
		/*
		 * assign set to get Iterator and iNoDuplicatesSize =
		 * resultNoDuplicates.size()
		 */
		...
		Iterator<Integer> iterNoDuplicatesIndexMode = hashRemoveDuplicates
				.values().iterator();
		...
		for (int index = 0; index < iNoDuplicatesSize; index++)
		{
			...
			selectedIndexMode[index] = iterNoDuplicatesIndexMode.next()
					.intValue();
		}
	}
	...
	// {
	// assert false : "setSelectedIndex() int [] updateIndex != int []
	// updateIndexMode !";
	// return;
	// }
	//
	...
	// {
	//
	// if (hashExternalId_2_HeatmapIndex.get(updateIndex[i]).intValue() !=
	// updateIndexMode[i])
	// {
	...
	// hashExternalId_2_HeatmapIndex.keySet().iterator();
	// Iterator<Integer> values =
	// hashExternalId_2_HeatmapIndex.values().iterator();
	//
	...
	protected void init_External2InternalHashMap() {
		...
		// refGeneralManager.getSingelton().logMsg("Can not load NCBI_GENEID
		// mapping!", LoggerType.MINOR_ERROR_XML);
		//			
		// return;
		// }
		//
		//
		// /* create HashMap now.. */
		// hashExternalId_2_HeatmapIndex_reverse = new HashMap<Integer,
		// Integer>();
		// Iterator<Integer> iter = hashNCBI_GENE2index.keySet().iterator();
		//	
		// for (int index=0; iter.hasNext(); index++) {
		// hashExternalId_2_HeatmapIndex_reverse.put(new Integer(index),
		// iter.next());
		// }

		hashExternalId_2_HeatmapIndex = new HashMap<Integer, Integer>(
				hashNCBI_GENEID_2_internalGraphVertexId.size());
		hashExternalId_2_HeatmapIndex_reverse = new HashMap<Integer, Integer>(
				hashNCBI_GENEID_2_internalGraphVertexId.size());

		Iterator<Integer> iter_InternalGraphVertexId = hashNCBI_GENEID_2_internalGraphVertexId
				.values().iterator();
	...
	protected final boolean processHits(final GL gl, final int iHitCount,
		...
		if (selectedIndexArray != null)
		{
			...
			notifyReceiver_PickedObject_singleSelection(selectedIndexArray,
					iSelectedMode_identifyier);

			// Integer iNCBIGeneID =
			// hashExternalId_2_HeatmapIndex_reverse.get(resultPickPointCoord[0]);
			// if ( iNCBIGeneID != null)
			// {
			// PathwayVertexGraphItem pickedGeneVertex;
			// pickedGeneVertex = (PathwayVertexGraphItem)
			// refGeneralManager.getSingelton(
		...
		// TODO: clean up this call, init from outside as soon as data is
		// available!
	...
	protected int[] addPickedPoint(ArrayList<Vec2f> fIndexPickedCoord,
		...
		if ((iSize % 2) == 1)
		{
			...
			/**
			 * Calculate all indices between left and right point and create an
			 * array with all these indices..
			 */
	...
	private void notifyReceiver_PickedObject_singleSelection(
			final int[] resultPickPointCoord, final int iModeValue) {
		...
		if (hashExternalId_2_HeatmapIndex_reverse != null)
		{
			...
			ArrayList<Integer> alSelectionId_PathwayId = new ArrayList<Integer>(
					resultPickPointCoord.length);

			for (int i = 0; i < resultPickPointCoord.length; i++)
			{
				/* convert internal index to external id.. */
				Integer lookupValue = hashExternalId_2_HeatmapIndex_reverse
						.get(resultPickPointCoord[i]);
				if (lookupValue == null)
				{
					return;
				}
				...
				while (iterList.hasNext())
				{
					...
					List<IGraph> list = bufferItem
							.getAllGraphByType(EGraphItemHierarchy.GRAPH_PARENT);
...
//							+ selectionSet.toString(), LoggerType.STATUS);
	...
	private void notifyReceiver_PickedObject_multipleSelection(
			final int[] resultPickPointCoord, final int iModeValue) {
		...
		int[] resultSelectedIndex = new int[selectedIndex.length
				+ resultPickPointCoord.length];
		int[] resultSelectedIndexMode = new int[selectedIndex.length
				+ resultPickPointCoord.length];
...
//					LoggerType.STATUS);
	...
	protected void pickObjects(final GL gl, final Point pickPoint) {
		...
		processHits(gl, iHitCount, iArPickingBuffer, pickPoint,
				fIndexPickedCoored);
...
//				"createHeatmap() create DsiplayList)", LoggerType.FULL);
	 ...
	 * @param gl
	 */
	private void render_picketPoints(GL gl, final float fIncY, final float fIncX) {
		...
		if ((selectedIndex.length > 0) || (!fIndexPickedCoored.isEmpty()))
		{
			...
			if (selectedIndex.length > 0)
			{
				...
				for (int index_X_H = 0; index_X_H < selectedIndex.length; index_X_H++)
				{
					...
					{
						/**
						 * Render one BOX on top, one Box below and connect them
						 * with 2 lines of each selected heatmap segment [X ,
						 * Y_min..Y_max]
						 */
						...
						/* BOTTOM */
						render_Selection_Detail(gl, fNowX, fNextX,
								viewingFrame[Y][MIN], fIncY,
								viewingFrame[Z][MIN],
								selectedIndexMode[index_X_H], -1.0f);
...
//	/**
//	 * 
//	 * @param fColorMappingShiftFromMean
//	 *            1.0f indicates no shift
//	 */
...
//			 */
//			colorMapperVec3f.addSamplingPoint_Vecf(new Vec3f(1, 0, 0),
//					(float) refMinMaxDataInteger.getMin(0));
//			colorMapperVec3f.addSamplingPoint_Vecf(new Vec3f(1, 1, 0),
//					(float) refMinMaxDataInteger.getMean(0));
//			colorMapperVec3f.addSamplingPoint_Vecf(new Vec3f(0, 1, 0),
//					(float) refMinMaxDataInteger.getMax(0));
//
...
//		colorMapperVec3f.addSamplingPoint_Vecf(new Vec3f(1, 1, 0),
//				(fColorMappingHighValue - fColorMappingLowValue) / 2);
//		colorMapperVec3f.addSamplingPoint_Vecf(new Vec3f(0, 1, 0),
//				fColorMappingHighValue);
//
//	}

	protected void colorMapping(final GL gl, final float fValue) {

		// colorMapperVec3f.colorMapping_glColor3f(gl, iValue);

		/* Use color mapper with LUT.. */
		Vec3f tmpNodeColor = colorMapper.colorMappingLookup(fValue);
...
//							LoggerType.MINOR_ERROR_XML);
	...
	public final void setTargetSetId(final int iTargetCollectionSetId) {
		...
		ISet addTargetSet = generalManager.getSetManager()
...
//						+ iTargetCollectionSetId + ") done!", LoggerType.FULL);
	...
	private void render_displaylistHeatmap_SortedStyle(GL gl) {
		...
		while (iterTargetSet.hasNext())
		{
			...
			IStorage refStorage = currentTargetSet
					.getStorageByDimAndIndex(0, 0);

			float[] dataArrayFloat = refStorage.getArrayFloat();
			...
			if (dataArrayFloat != null)
			{
				...
				for (int iIndex_X = 0; iter.hasNext(); iIndex_X++)
				{
					if (iIndex_X >= iRenderIndexStart)
					{
						...
						colorMapping(gl, dataArrayFloat[iter.next()]);
	...
	private void render_Selection_Detail(GL gl, final float fX,
			final float fX_next, final float fY, final float fIncY,
			final float fZ, final int selectedIndexMode_Value, final float fSign) {
		...
		float fY_a = fY + selectedIndexMode_Value * fIncY * fSelectionPixel
				* fSign;
	...
	private void render_Selection_ownArea(GL gl, final float fIncY) {
		...
		float fOffsetY = 2.0f; // (viewingFrame[X][MAX] -
								// viewingFrame[X][MIN]);
		...
		float fXOffset = ((viewingFrame[X][MAX] - viewingFrame[X][MIN]) - (selectedIndex.length + 1)
				* fIncX) * 0.5f;
		...
		/**
		 * sort selected index sort selected index in each view; preserve
		 * mapping <selected_index,selected_index_mode> using a HashMap
		 */
		HashMap<Integer, Integer> hashPreservMapping_selectedIndex_2_selectedIndexMode = new HashMap<Integer, Integer>();

		for (int i = 0; i < selectedIndex.length; i++)
		{
			hashPreservMapping_selectedIndex_2_selectedIndexMode.put(
					selectedIndex[i], selectedIndexMode[i]);
		}
		...
		/**
		 * end: sort selectedIndex array (int[]) selectedIndex[] is sorted now
		 */
		...
		for (int i = 0; i < selectedIndex.length; i++)
		{
			...
			Iterator<Integer> iterRawDataArrays_Offset = bufferValueArrays_Offset
					.iterator();
			...
			while (iterRawDataArrays.hasNext())
			{
				...
				int iCurrentIndex_InArray = iterRawDataArrays_Offset.next()
						.intValue()
						+ iCurrentIndex;
			...
			if (bEnableGroupHighlighting)
			{
				...
				int selectedIndexMode_currentValue = hashPreservMapping_selectedIndex_2_selectedIndexMode
						.get(iCurrentIndex).intValue();

				if (selectedIndexMode_currentValue > 0)
				{
					// if (selectedIndexMode[i] > 0 ) {

					render_Selection_Detail(gl, fNowX, fNextX, fNowY, fIncY,
							viewingFrame[Z][MIN],
							selectedIndexMode_currentValue, 1.0f);

				} // if (selectedIndexMode[i] > 0 ) {
			}

			/** render connection line to target in heatmap */
			int iRenderTargetLine_Index = iCurrentIndex
					- this.iRenderIndexStart;

			float fLineX = viewingFrame[X][MIN] + fIncX
					* iRenderTargetLine_Index + fIncX_halfSize;

			float fLineXleft = viewingFrame[X][MIN] + fIncX
					* iRenderTargetLine_Index;
			float fLineXright = viewingFrame[X][MIN] + fIncX
					* (iRenderTargetLine_Index + 1);
					...
					&& (iCurrentIndex < this.iRenderIndexStop))
			{
				...
				if (bShowSelectionItemBoundaries)
				{
					...
					if (bBuffer_ShowSelectionItemBoundaries)
					{
						...
						{
							/**
							 * put right line in buffer... overwrite buffer also
							 */
						...
						} else
						{
							...
							gl.glVertex3f(fBuffer_ShowSelectionItemBoundaries,
									viewingFrame[Y][MIN], viewingFrame[Z][MIN]);
							gl.glVertex3f(fBuffer_ShowSelectionItemBoundaries,
									viewingFrame[Y][MAX], viewingFrame[Z][MIN]);
							gl.glEnd();
							...
							// gl.glColor3f(1, 0, 0);
							gl.glBegin(GL.GL_LINES);
							gl.glVertex3f(fLineXleft, viewingFrame[Y][MIN],
									viewingFrame[Z][MIN]);
							gl.glVertex3f(fLineXleft, viewingFrame[Y][MAX],
									viewingFrame[Z][MIN]);
							gl.glEnd();
					...
					{
						/*
						 * render separation line in own render area for
						 * selection
						 */
						color_Selection_Frame.glColor3f(gl);
						gl.glBegin(GL.GL_LINES);

						gl.glVertex3f(fNowX, viewingFrame[Y][MIN] + fOffsetY,
								viewingFrame[Z][MIN]);
						gl.glVertex3f(fNowX, fNowY, viewingFrame[Z][MIN]);
			...
			{
				if (bShowSelectionItemBoundaries
						&& bBuffer_ShowSelectionItemBoundaries_RenderSelectionArea_FinishLine)
				{
			...
			gl.glVertex3f(fNowX + fIncX_halfSize, viewingFrame[Y][MIN]
					+ fOffsetY - fSelectionPixel * fIncY, viewingFrame[Z][MIN]);
			gl.glVertex3f(fLineX, viewingFrame[Y][MAX] + fSelectionPixel
					* fIncY, viewingFrame[Z][MIN]);
			...
			gl.glVertex3f(fNowX + fIncX_halfSize, viewingFrame[Y][MIN]
					+ fOffsetY - fSelectionPixel * fIncY, viewingFrame[Z][MIN]);
			gl.glVertex3f(fNowX + fIncX, viewingFrame[Y][MIN] + fOffsetY,
					viewingFrame[Z][MIN]);
			gl.glEnd();
			/**
			...
			gl.glVertex3f(fLineX, viewingFrame[Y][MAX] + fSelectionPixel
					* fIncY, viewingFrame[Z][MIN]);
			gl.glVertex3f(fLineXright, viewingFrame[Y][MAX],
					viewingFrame[Z][MIN]);
			gl.glEnd();
			...
			gl.glVertex3f(fLineXleft, viewingFrame[Y][MIN],
					viewingFrame[Z][MIN]);
			gl.glVertex3f(fLineX, viewingFrame[Y][MIN] - fSelectionPixel
					* fIncY, viewingFrame[Z][MIN]);
			gl.glVertex3f(fLineXright, viewingFrame[Y][MIN],
					viewingFrame[Z][MIN]);
		...
		color_Selection_Frame.glColor3f(gl);
		gl.glBegin(GL.GL_LINE_LOOP);
		gl.glVertex3f(viewingFrame[X][MIN] + fXOffset, viewingFrame[Y][MIN]
				+ fOffsetY, viewingFrame[Z][MIN]);
		gl.glVertex3f(fNowX, viewingFrame[Y][MIN] + fOffsetY,
				viewingFrame[Z][MIN]);
		gl.glVertex3f(fNowX, viewingFrame[Y][MAX] + fOffsetY,
				viewingFrame[Z][MIN]);
		gl.glVertex3f(viewingFrame[X][MIN] + fXOffset, viewingFrame[Y][MAX]
				+ fOffsetY, viewingFrame[Z][MIN]);
		gl.glEnd();
	...
	private void render_displaylistHeatmap_FlatStyle(GL gl) {
		...
		while (iterTargetSet.hasNext())
		{
			...
			float[] f_dataValues = refStorage.getArrayFloat();

			if (f_dataValues != null)
			{
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.caleydo.core.view.opengl.canvas.heatmap.IGLCanvasHeatmap2D#setSelectionItems(int[],
	 *      int[], int[], int[])
	 */
...
//						+ updatedSet.toString(), LoggerType.STATUS);
		...
		for (int j = 0; j < intBuffer.length; j++)
		{
			...
			Iterator<IGraphItem> iterGraphItemRep = ((IGraphItem) generalManager
					.getPathwayItemManager().getItem(intBuffer[j])).getAllItemsByProp(
							EGraphItemProperty.ALIAS_PARENT).iterator();

			while (iterGraphItemRep.hasNext())
			{
				/* convert external id to internal.. */
				Integer indexInt = hashExternalId_2_HeatmapIndex
						.get(iterGraphItemRep.next().getId());

				if (indexInt != null)
				{
					System.err
							.println("GLCanvasHeatmap2DColumn receiveUpdate ["
									+ intBuffer[j] + "=>" + indexInt + "], ");
		...
		{
			System.err
					.println("GLCanvasHeatmap2DColumn receiveUpdate []    COPY to Heatmap-Selection.. ");
	 ...
	 *            id of GLcanvasUser
	 * @param pos
	 *            new position of window; if null window is removed
	 */
	public void setChildWindow(final int idChild, final Vec4i pos) {
		...
		{
			Iterator<GLCanvasHeatmap2DColumn> iter = alExternalWindow_Link2Parent
					.iterator();

			while (iter.hasNext())
			{
				GLCanvasHeatmap2DColumn parent = iter.next();
				parent.setChildWindow(this.getId(), new Vec4i(
						iRenderIndexStart, iRenderIndexStop, 0, 3));
	...
	private void render_ExternalChildrenWindows(GL gl, final float fXinc,
			final float fYinc) {
		...
		{
			/**
			 * This is a child window! ==> no rendering!
			 */
			return;
		}
	...
	public void addLinkTo_ParentWindow(int id) {
		...
		GLCanvasHeatmap2DColumn parentHeatmap = (GLCanvasHeatmap2DColumn) generalManager
				.getViewGLCanvasManager().getItem(id);
	...
	public void addLinkTo_ParentWindow(GLCanvasHeatmap2DColumn parent) {
		...
		parent.setChildWindow(this.getId(), new Vec4i(iRenderIndexStart,
				iRenderIndexStop, 0, 3));
	}
