...
/**
 * This class defines a data type
 * It uses GlyphAttributeGroup to combines more than one data type into one
 * 
 * @author Stefan Sauer
 *
 */
public class GlyphAttributeType {
	...
	public GlyphAttributeType(final IGeneralManager generalManager, String name, int externalColumnIndex)
	{
...
/*
	...
	{
		if(!hmDistribution.containsKey(index)) hmDistribution.put(index, 0);
	...
	public void decDistribution(int index)
	{
		...
		hmDistribution.put(index, hmDistribution.get(index)-1);
		if(hmDistribution.get(index) < 0) hmDistribution.put(index, 0);
	}
	
	public void incSelectedDistribution(int index)
	{
		if(!hmSelectedDistribution.containsKey(index)) hmSelectedDistribution.put(index, 0);
	...
	public void decSelectedDistribution(int index)
	{
		...
		hmSelectedDistribution.put(index, hmSelectedDistribution.get(index)-1);
		if(hmSelectedDistribution.get(index) < 0) hmSelectedDistribution.put(index, 0);
	}
...
/*		
		Set<Integer> ks = hmDistribution.keySet();
		
		for(Integer k : ks) {
			int v = hmDistribution.get(k);
			int s = 0;
			if(hmSelectedDistribution.containsKey(k)) s = hmSelectedDistribution.get(k);
