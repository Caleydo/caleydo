...
implements MouseListener, MouseMotionListener, MouseWheelListener {
	...
	public void render(GL gl) 
	{
		...
		//System.out.println(fAspectRatio + " => " + viewport[0] + ", " + viewport[1] + ", " + viewport[2] + ", " + viewport[3] );
...
//		System.out.println( fAspectRatio); 
		...
		if( iRubberBandStartX != 0 && iRubberBandStartY != 0) {
			...
			float xc = (iRubberBandCurrentX-offsetX) * (fAspectRatio/10.0f) * (camZScale + 1.0f);
			float yc = (iRubberBandCurrentY-offsetY) * (fAspectRatio/10.0f) * (camZScale + 1.0f);
		
		gl.glTranslatef(xs-1,-ys-1, 0);
		
		GLSharedObjects.drawAxis(gl);
		
		gl.glTranslatef( (xc - xs),
				         -(yc - ys), 0);
