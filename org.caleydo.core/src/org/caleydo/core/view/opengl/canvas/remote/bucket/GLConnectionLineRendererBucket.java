...
public class GLConnectionLineRendererBucket
	...
	protected void renderLineBundling(final GL gl, EIDType idType, float[] fArColor) {
		...
		ArrayList<ArrayList<ArrayList<Vec3f>>> connectionLinesAllViews =
			new ArrayList<ArrayList<ArrayList<Vec3f>>>(4);
		...
		for (Integer iKey : keySet) {
			...
			// renderLine(gl, vecViewBundlingPoint, currentPoint, 0, hashViewToCenterPoint.get(iKey),
			// fArColor);

			for (Vec3f currentPoint : depthSort(pointsToDepthSort)) {
				if (activeViewID != -1 && iKey == activeViewID)
					connectionLinesActiveView.add(createControlPoints(vecViewBundlingPoint, currentPoint,
						hashViewToCenterPoint.get(iKey)));
				else
					connectionLinesOtherViews.add(createControlPoints(vecViewBundlingPoint, currentPoint,
						hashViewToCenterPoint.get(iKey)));
			}
