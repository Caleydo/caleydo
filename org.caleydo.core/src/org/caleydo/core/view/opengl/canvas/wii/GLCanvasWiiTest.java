 ...
 * the help of OpenGL. Currently only the movement of WiiRemote is displayed, but
 * adding support for the Nunchuk is easily done.
 * </p>
 * <p>
 * I currently aware of display issue, based on the rotation angle of the remote,
 * which I need to fix. Inexperience with OpenGL has so far prevented me from
 * resolving the issue.
 * </p>
 * <p>
 * In order to use this class you need to have the following libraries to
 * build and run this project:
 * <ul>
 *   <li> JOGL ( https://jogl.dev.java.net/ )
 *   <li> WiiRemoteJ ( http://www.wiili.org/WiiremoteJ )
 *   <li> An implemention of the Java Bluetooth API ( see http://www.wiili.org/WiiremoteJ )
 * </ul>
 * <p> 
 * @author Andre-John Mas
 *
 */
public class GLCanvasWiiTest extends AGLCanvasUser implements Runnable,
        KeyListener {
     ...
     * Constructor.
     */
    public GLCanvasWiiTest(final IGeneralManager generalManager,
			final int iViewId,
...
//        new Thread(this).start();
    ...
    private void displayBox(GL gl, float x, float y, float z, float width,
    ...
    private void drawCircle(GL gl, float radius, float x, float y, float z) {
        ...
        for (int i = 0; i < 360; i++) {
            ...
            gl.glVertex3d(x + Math.cos(angle) * radius, y + Math.sin(angle)
                    * radius, z);
        }
    ...
    private void drawGraph(GL gl, int[] samples, int sampleOffset, float x,
            float z) {
    ...
    public void display(final GL gl) {
        ...
        if (this.pointerVisible) {
            ...
            drawCircle(gl, 0.5f,
                    (float) (left + (screenWidth * (1.0 - this.pointerX))),
                    (float) (bottom + (screenHeight * (1.0 - this.pointerY))),
                    -8.0f);
        }
...
//        glu.gluPerspective(45.0f, width / (height * 1.0f), 0.1f, 100.0f); // Calculate
//                                                                            // The
//                                                                            // Aspect
//                                                                            // Ratio
//                                                                            // Of
