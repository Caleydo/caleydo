...
public class GLConsecutiveConnectionGraphDrawing
	...
	protected void renderLineBundling(final GL gl, EIDType idType, float[] fArColor) {
		...
		for (Integer iKey : keySet) {
			...
			else if (iKey == parCoordID && multiplePoints){
	...
	/*	heatmapPredecessor.clear();
		...
		*/
		...
		int[] parCoordIDs = {parCoordsPredecessorID, parCoordsSucessorID};
		int[] heatMapIDs = {heatMapPredecessorID, heatMapSuccessorID};
		...
		//getting ids of pre/suc of heatmap/parcoords
		calculatePredecessorAndSuccessor(parCoordIDs, heatMapIDs, hashViewToCenterPoint);
		heatMapPredecessorID = heatMapIDs[0];
		heatMapSuccessorID = heatMapIDs[1];
		parCoordsPredecessorID = parCoordIDs[0];
		parCoordsSucessorID = parCoordIDs[1];
		
		
		getPointsFromHeatMapAndParCoords(heatMapPredecessorID, heatMapSuccessorID, heatMapPoints, parCoordsPredecessorID, parCoordsSucessorID, parCoordsPoints, hashViewToCenterPoint);
		...
		for (Integer iKey : keySet) {
			...
			if (heatmapSuccessor.size() > 0){
				...
				for(Vec3f currentPoint : depthSort(pointsToDepthSort)){
					if (heatMapSuccessorID == parCoordID)
						vecCenter = calculateControlPoint(heatmapSuccessor.get(0).get(0), parCoordsPredecessor.get(0).get(0));
					else
						vecCenter = calculateControlPoint(heatmapSuccessor.get(0).get(0), hashViewToCenterPoint.get(heatMapSuccessorID));
					connectionLinesCurrentView.addLine( createControlPoints( calculateBundlingPoint(calculateCenter(heatmapSuccessor), vecCenter), currentPoint, calculateCenter(heatmapSuccessor) ) );
			...
			if (iKey != heatMapID && iKey != parCoordID){
				...
				for(Vec3f currentPoint : depthSort(pointsToDepthSort)){
					if (iKey == activeViewID){
						...
						temp.addLine( createControlPoints(vecViewBundlingPoint, currentPoint, hashViewToCenterPoint.get(iKey) ) );
					}
				}
				pointsToDepthSort.clear();
			}
			...
			if (heatMapSuccessorID == parCoordID)
				anim.addLine(createControlPoints(parCoordsPredecessor.get(0).get(0), heatmapSuccessor.get(0).get(0), calculateControlPoint(heatmapSuccessor.get(0).get(0), parCoordsPredecessor.get(0).get(0))));
			else
				anim.addLine(createControlPoints(hashViewToCenterPoint.get(heatMapSuccessorID), heatmapSuccessor.get(0).get(0), new Vec3f(2,2,0)));
			connectionLinesAllViews.add(anim);
			anim = new VisLinkAnimationStage();
			if (parCoordsSucessorID!=-1){
				anim.addLine(createControlPoints(hashViewToCenterPoint.get(parCoordsSucessorID), parCoordsSuccessor.get(0).get(0), calculateControlPoint(parCoordsSuccessor.get(0).get(0), hashViewToCenterPoint.get(parCoordsSucessorID))));
				connectionLinesAllViews.add(anim);
				if (hashIDTypeToViewToPointLists.get(idType).get(parCoordsSucessorID).size()>1)
					connectionLinesAllViews.add(temp);
			}
			else
				anim.addLine(createControlPoints(parCoordsPredecessor.get(0).get(0), hashViewToCenterPoint.get(parCoordsPredecessorID), new Vec3f(-2,2,0)));
		...
		*/	
		...
		if (multiplePoints){
			if(ids.size()>1){
				...
				renderLineOnlyOneOtherViewOnStackAvailable(remoteId, idType, hashViewToCenterPoint, bundlingPoints, connections, connectionLinesAllViews);
			}
		}
		else{
			if(ids.size()>1){
	...
	private void getPointsFromHeatMapAndParCoords(int heatMapPredecessorID, int heatMapSuccessorID,
			ArrayList<ArrayList<Vec3f>> heatMapPoints, int parCoordsPredecessorID, int parCoordsSucessorID, ArrayList<ArrayList<Vec3f>> parCoordsPoints, HashMap<Integer, Vec3f> hashViewToCenterPoint) {
		...
		if (heatMapSuccessorID != parCoordID){
			...
			if (parCoordsPredecessorID != -1 && parCoordsSucessorID != -1){
				...
				for (ArrayList<Vec3f> parCoordsList : parCoordsPoints) {
					...
					Vec3f distanceToPredecessor = predecessorPoint.minus(hashViewToCenterPoint.get(parCoordsPredecessorID));
					Vec3f distanceToSuccessor = successorPoint.minus(hashViewToCenterPoint.get(parCoordsSucessorID));
			...
			if (parCoordsSucessorID != -1){
				...
				for (ArrayList<Vec3f> parCoordsList : parCoordsPoints) {
					for (ArrayList<Vec3f> heatMapList : heatMapPoints) {
						...
						Vec3f distanceToSuccessor = successorPoint.minus(parCoordsList.get(0));
						currentSuccessorPath = distanceToSuccessor.length();
						currentPredecessorPath = distanceToPredecessor.length();
						if (currentPredecessorPath < minPredecessorPath){
							minPredecessorPath = currentPredecessorPath;
	...
	private void calculatePredecessorAndSuccessor(int[] parCoordIDs, int[] heatMapIDs, HashMap<Integer, Vec3f> hashViewToCenterPoint) {
		if (focusLevel.getElementByPositionIndex(0).getGLView() instanceof GLHeatMap){
			for (int stackCount = 0; stackCount < stackLevel.getCapacity(); stackCount++){
				//first element are parCoords
				if (stackLevel.getElementByPositionIndex(stackCount).getGLView() != null && stackLevel.getElementByPositionIndex(stackCount).getGLView().getID() == parCoordID){
					heatMapIDs[1] = parCoordID;
					parCoordIDs[0] = heatMapID;
					if (stackCount < stackLevel.getCapacity()-2){
						for (int innerCount = stackCount+1; innerCount < stackLevel.getCapacity(); innerCount++){
							if (stackLevel.getElementByPositionIndex(innerCount).getGLView()!=null && hashViewToCenterPoint.containsKey(stackLevel.getElementByPositionIndex(innerCount).getGLView().getID())){
								if (innerCount - stackCount == 2)
									gap = true;
								parCoordIDs[1] = stackLevel.getElementByPositionIndex(innerCount).getGLView().getID();
								break;
							}
						}
					}
				...
				//first element are not parCoords
				else if (stackLevel.getElementByPositionIndex(stackCount).getGLView() != null && hashViewToCenterPoint.containsKey(stackLevel.getElementByPositionIndex(stackCount).getGLView().getID())){
					heatMapIDs[1] = stackLevel.getElementByPositionIndex(stackCount).getGLView().getID();
					//get pre and suc of parCoords if they are available 
					if (parCoordID != -1){
						for (int count = stackCount+1; count < stackLevel.getCapacity(); count++) {
							if (stackLevel.getElementByPositionIndex(count).getGLView() instanceof GLParallelCoordinates){
								for (int innerCount = count-1; innerCount >= 0; innerCount--){
									if (stackLevel.getElementByPositionIndex(innerCount).getGLView() != null && hashViewToCenterPoint.containsKey(stackLevel.getElementByPositionIndex(innerCount).getGLView().getID())){
										parCoordIDs[0] = stackLevel.getElementByPositionIndex(count-1).getGLView().getID();
										break;
									}
								}
							}
							if (count != stackLevel.getCapacity()-1){
								for (int innerCount = count++; innerCount < stackLevel.getCapacity(); innerCount++){
									if (stackLevel.getElementByPositionIndex(innerCount).getGLView() != null && hashViewToCenterPoint.containsKey(stackLevel.getElementByPositionIndex(innerCount).getGLView().getID())){
										parCoordIDs[1] = stackLevel.getElementByPositionIndex(innerCount).getGLView().getID();
										break;
									}
			...
			for (int stackCount = 0; stackCount < stackLevel.getCapacity(); stackCount++){
				if (stackLevel.getElementByPositionIndex(stackCount).getGLView() != null && stackLevel.getElementByPositionIndex(stackCount).getGLView().getID() == heatMapID){
					parCoordIDs[1] = heatMapID;
					heatMapIDs[0] = parCoordID;
					if (stackCount < stackLevel.getCapacity()-2){
						for (int innerCount = stackCount+1; innerCount < stackLevel.getCapacity(); innerCount++){
							if (stackLevel.getElementByPositionIndex(innerCount).getGLView()!=null && hashViewToCenterPoint.containsKey(stackLevel.getElementByPositionIndex(innerCount).getGLView().getID())){
								if (innerCount - stackCount == 2)
									gap = true;
								heatMapIDs[1] = stackLevel.getElementByPositionIndex(innerCount).getGLView().getID();
								break;
							}
						}
					}
					...
					parCoordIDs[1] = stackLevel.getElementByPositionIndex(stackCount).getGLView().getID();
					break;
				}
			}
		}
	...
	/** selects which lines have to be rendered if the only two views involved are the focus element and one stack element 
	 * 
	 * @param remoteId
	 * @param idType
	 ...
	 */
	private void renderLineOnlyOneOtherViewOnStackAvailable(int remoteId, EIDType idType, HashMap<Integer, Vec3f> hashViewToCenterPoint, HashMap<Integer, Vec3f> bundlingPoints, HashMap<Integer, VisLinkAnimationStage> connections, ArrayList<VisLinkAnimationStage> connectionLinesAllViews) {
		VisLinkAnimationStage bundlingLine = new VisLinkAnimationStage(true);
		if (activeViewID == heatMapID){
			if (hashIDTypeToViewToPointLists.get(idType).get(remoteId).size()>1){
				if (multiplePoints)
					bundlingLine.addLine(createControlPoints(calculateBundlingPoint(calculateCenter(heatmapSuccessor), vecCenter), bundlingPoints.get(remoteId), vecCenter));	
				else
					bundlingLine.addLine(createControlPoints(heatmapSuccessor.get(0).get(0), bundlingPoints.get(remoteId), vecCenter));
			}
			else{
				if (multiplePoints)
					bundlingLine.addLine(createControlPoints(calculateBundlingPoint(calculateCenter(heatmapSuccessor), vecCenter), hashViewToCenterPoint.get(remoteId), vecCenter));			
				else
			...
			if (hashIDTypeToViewToPointLists.get(idType).get(remoteId).size()>1){
				if (multiplePoints)
					bundlingLine.addLine(createControlPoints(calculateBundlingPoint(calculateCenter(parCoordsSuccessor), vecCenter), bundlingPoints.get(remoteId), vecCenter));			
				else
					bundlingLine.addLine(createControlPoints(parCoordsSuccessor.get(0).get(0), bundlingPoints.get(remoteId), vecCenter));
			}
			else{
				if (multiplePoints)
					bundlingLine.addLine(createControlPoints(calculateBundlingPoint(calculateCenter(parCoordsSuccessor), vecCenter), hashViewToCenterPoint.get(remoteId), vecCenter));			
				else
			...
			if (hashIDTypeToViewToPointLists.get(idType).get(activeViewID).size()>1){
				if (multiplePoints)
					bundlingLine.addLine(createControlPoints(bundlingPoints.get(activeViewID), calculateBundlingPoint(calculateCenter(heatmapPredecessor), vecCenter), vecCenter));
				else
	...
	private void renderFromStackLevel(GL gl, HashMap<Integer, VisLinkAnimationStage> connections, HashMap<Integer, Vec3f> bundlingPoints, EIDType idType, HashMap<Integer, Vec3f> hashViewToCenterPoint){
		...
		if (multiplePoints){
			if (((ids.size() > 1) && (focusID ==-1)) || ((ids.size() >2) && (focusID != -1))){
				...
				if (focusLevel.getElementByPositionIndex(0).getGLView() != null){
					renderLineOnlyOneOtherViewOnFocusAvailable(idType, hashViewToCenterPoint, bundlingPoints, connections, connectionLinesAllViews);
				}	
			}
		}
		else{
	...
	/** Method that selects the lines to be drawn if the only view except the active view is located in focus level
	 * 
	 * @param remoteId
	 * @param idType 
	 * @param hashViewToCenterPoint
	 ...
	 * @param connectionLinesAllViews
	 */
	private void renderLineOnlyOneOtherViewOnFocusAvailable(EIDType idType, HashMap<Integer, Vec3f> hashViewToCenterPoint, HashMap<Integer, Vec3f> bundlingPoints, HashMap<Integer, VisLinkAnimationStage> connections, ArrayList<VisLinkAnimationStage> connectionLinesAllViews) {
		VisLinkAnimationStage bundlingLine = new VisLinkAnimationStage(true);
		...
		if (remoteId == heatMapID){
			...
			if (hashIDTypeToViewToPointLists.get(idType).get(activeViewID).size() == 1){
				if (multiplePoints){
					bundlingLine.addLine(createControlPoints(hashViewToCenterPoint.get(activeViewID), calculateBundlingPoint(calculateCenter(heatmapSuccessor), vecCenter), vecCenter));
					connectionLinesAllViews.add(bundlingLine);
					connectionLinesAllViews.add(heatMapStage);
				}
				else{
					bundlingLine.addLine(createControlPoints(hashViewToCenterPoint.get(activeViewID), heatmapSuccessor.get(0).get(0), vecCenter));
					connectionLinesAllViews.add(bundlingLine);
				}
			}
			else{
				if (multiplePoints){
					...
					bundlingLine.addLine(createControlPoints(bundlingPoints.get(activeViewID), heatmapSuccessor.get(0).get(0), vecCenter));
					connectionLinesAllViews.add(connections.get(activeViewID));
					connectionLinesAllViews.add(bundlingLine);
				}
			}	
			...
			if (hashIDTypeToViewToPointLists.get(idType).get(activeViewID).size() == 1){
				if (multiplePoints){
					bundlingLine.addLine(createControlPoints(hashViewToCenterPoint.get(activeViewID), calculateBundlingPoint(calculateCenter(parCoordsSuccessor), vecCenter), vecCenter));
					connectionLinesAllViews.add(bundlingLine);
					connectionLinesAllViews.add(parCoordStage);
				}
				else{
					bundlingLine.addLine(createControlPoints(hashViewToCenterPoint.get(activeViewID), parCoordsSuccessor.get(0).get(0), vecCenter));
					connectionLinesAllViews.add(bundlingLine);
				...
				if (multiplePoints){
					...
					bundlingLine.addLine(createControlPoints(bundlingPoints.get(activeViewID), parCoordsSuccessor.get(0).get(0), vecCenter));
					connectionLinesAllViews.add(connections.get(activeViewID));
					connectionLinesAllViews.add(bundlingLine);
				}
			}
			...
			if ((hashIDTypeToViewToPointLists.get(idType).get(activeViewID).size() == 1) && (hashIDTypeToViewToPointLists.get(idType).get(remoteId).size() == 1)){
				...
				if (activeViewID == heatMapID)
					...
					bundlingLine.addLine(createControlPoints(hashViewToCenterPoint.get(activeViewID), bundlingPoints.get(activeViewID), vecCenter));
				connectionLinesAllViews.add(bundlingLine);
				connectionLinesAllViews.add(connections.get(remoteId));
			}
	...
	protected ArrayList<ArrayList<Vec3f>> calculateOptimalSinglePoints(
		...
		if (isHeatMapCentered)
			optimalDynamicPoints = getOptimalSingleHeatMapPointsCenter(hashViewToCenterPoint, heatMapPoints, parCoordsPoints);
		else if (areParCoordsCentered)
			optimalDynamicPoints = getOptimalSingleParCoordPointsCenter(hashViewToCenterPoint, heatMapPoints, parCoordsPoints);
		else
			optimalDynamicPoints = getOptimalSinglePointsStack(hashViewToCenterPoint, heatMapPoints, parCoordsPoints);
		
		return optimalDynamicPoints;
	}
	
	
	private ArrayList<ArrayList<Vec3f>> getOptimalSinglePointsStack(HashMap<Integer, Vec3f> hashViewToCenterPoint, ArrayList<ArrayList<Vec3f>> heatMapPoints, ArrayList<ArrayList<Vec3f>> parCoordsPoints) {
		...
		if (heatMapSuccessorID == parCoordID){
			...
			for (ArrayList<Vec3f> heatMapList : heatMapPoints) {
				...
				if (heatMapPredecessorID == -1){
					...
					if (tempID != heatMapSuccessorID && tempID != -1 && hashViewToCenterPoint.containsKey(tempID))
	 ...
	 * @return
	 */
	private ArrayList<ArrayList<Vec3f>> getOptimalSingleParCoordPointsCenter(HashMap<Integer, Vec3f> hashViewToCenterPoint, ArrayList<ArrayList<Vec3f>> heatMapPoints, ArrayList<ArrayList<Vec3f>> parCoordsPoints) {
		ArrayList<ArrayList<Vec3f>> pointsList = new ArrayList<ArrayList<Vec3f>>();
		ArrayList<Vec3f> optimalHeatMapPredecessor = new ArrayList<Vec3f>();
		ArrayList<Vec3f> optimalHeatMapSucessor = new ArrayList<Vec3f>();
	...
	private ArrayList<ArrayList<Vec3f>> getOptimalSingleHeatMapPointsCenter(HashMap<Integer, Vec3f> hashViewToCenterPoint, ArrayList<ArrayList<Vec3f>> heatMapPoints, ArrayList<ArrayList<Vec3f>> parCoordsPoints) {
		ArrayList<ArrayList<Vec3f>> pointsList = new ArrayList<ArrayList<Vec3f>>();
	...
	protected ArrayList<ArrayList<ArrayList<Vec3f>>> calculateOptimalMultiplePoints(
		...
		if (isHeatMapCentered)
			optimalDynamicPoints = getOptimalMultipleHeatMapPointsCenter(hashViewToCenterPoint, multipleHeatMapPoints, heatMapCenterPoints, multipleParCoordPoints, parCoordCenterPoints);
		else if (areParCoordsCentered)
			optimalDynamicPoints = getOptimalMultipleParCoordPointsCenter(hashViewToCenterPoint, multipleHeatMapPoints, heatMapCenterPoints, multipleParCoordPoints, parCoordCenterPoints);
		else
			optimalDynamicPoints = getOptimalMultiplePointsStack(hashViewToCenterPoint, multipleHeatMapPoints, heatMapCenterPoints, multipleParCoordPoints, parCoordCenterPoints);
	...
	private ArrayList<ArrayList<ArrayList<Vec3f>>> getOptimalMultiplePointsStack(
		...
		ArrayList<Vec3f> heatMapCenterPoints, ArrayList<ArrayList<ArrayList<Vec3f>>> multipleParCoordPoints, ArrayList<Vec3f> parCoordCenterPoints) {
	...
	private ArrayList<ArrayList<ArrayList<Vec3f>>> getOptimalMultipleParCoordPointsCenter(
		HashMap<Integer, Vec3f> hashViewToCenterPoint, ArrayList<ArrayList<ArrayList<Vec3f>>> multipleHeatMapPoints,
		ArrayList<Vec3f> heatMapCenterPoints2, ArrayList<ArrayList<ArrayList<Vec3f>>> multipleParCoordPoints, ArrayList<Vec3f> parCoordCenterPoints2) {
		...
		if (isHeatMap) {
			...
			if (heatMapOnStackAndHasSucessor && !isGapParCoordsCenteredAndHeatMapOnStack){
				...
				for (Vec3f parCoordsList : parCoordCenterPoints) {
					...
					if (currentPath < minPath) {
						minPath = currentPath;
						heatmapSuccessor = multipleParCoordPoints.get(parCoordCenterPoints.indexOf(parCoordsList));
					}
				}
			}
		}
		...
		if (areParCoords) {
			...
			if (parCoordsOnStackAndHaveSucessor && !isGapParCoordsOnStackAndHeatMapCentered){
				...
				for (Vec3f heatMapList : heatMapCenterPoints) {
					...
					if (currentPath < minPath) {
						minPath = currentPath;
						parCoordsSuccessor = multipleHeatMapPoints.get(heatMapCenterPoints.indexOf(heatMapList));
					}
				}			
			}
		}
