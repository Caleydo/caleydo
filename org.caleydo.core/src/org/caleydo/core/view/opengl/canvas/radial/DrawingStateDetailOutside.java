...
public class DrawingStateDetailOutside
	 ...
	 *            Map is filled with key-value pairs where the key is a partial disc that has a highlighted
	 *            child indicator and its value is the corresponding drawing strategy.
	 */
		...
		if (pdCurrentMouseOverElement != null) {
			...
			if (isPartialDiscInDetailOutside(pdCurrentMouseOverElement)) {
				pdCurrentMouseOverElement.decoratePDDrawingStrategyChildren(dsLabelDecorator, Math.min(
					RadialHierarchyRenderStyle.MAX_LABELING_DEPTH, iDisplayedOverviewDepth));
			}
			else {
	...
	public void draw(float fXCenter, float fYCenter, GL gl, GLU glu) {
		...
		HashMap<PartialDisc, APDDrawingStrategyChildIndicator> mapChildIndicatedStrategies =
			new HashMap<PartialDisc, APDDrawingStrategyChildIndicator>();
		parentIndicatorType = ESelectionType.NORMAL;

		boolean bContinueDrawing =
			initDrawingStrategies(mapSelectedDrawingStrategies, mapChildIndicatedStrategies);
		if (!bContinueDrawing) {
		...
		if (pdSelected != pdRealRootElement && pdSelected.hasChildren()) {
			...
			if (pdSelected == pdCurrentRootElement) {
				...
				pdCurrentSelectedElement.setPDDrawingStrategyChildren(radialHierarchy
					.getDrawingStrategyManager().getDefaultDrawingStrategy(), iDisplayedDetailViewDepth);
