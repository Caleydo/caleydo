...
public class GLOffScreenTextureRenderer {
	...
	public void renderRubberBucket(final GL gl, RemoteLevel stackLevel,
		BucketLayoutRenderStyle bucketLayoutRenderStyle, GLRemoteRendering glRemoteRendering) {
		...
		if (stackLevel.getElementByPositionIndex(0).getContainedElementID() != -1) {
			...
			gl.glTexCoord2f(0, 1);
			gl.glVertex3f(-BucketLayoutRenderStyle.BUCKET_WIDTH, BucketLayoutRenderStyle.BUCKET_HEIGHT,
	...
	private int emptyTexture(GL gl) {
		...
		gl.glTexImage2D(GL.GL_TEXTURE_2D, 0, 4, 1024, 1024, 0, GL.GL_RGBA, GL.GL_UNSIGNED_BYTE, data);
