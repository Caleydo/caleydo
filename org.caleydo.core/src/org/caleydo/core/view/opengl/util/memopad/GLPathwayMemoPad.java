...
public class GLPathwayMemoPad
{
	...
	private RemoteElementManager memoPad;
	...
	{
		memoPad = new RemoteElem(EHierarchyLevel.MEMO);
	...
	public void init(final GL gl)
	{
		...
		for (int iMemoIndex = 0; iMemoIndex < memoPad.getCapacity(); iMemoIndex++)
		{
			...
			transform.setScale(new Vec3f(SCALING_FACTOR_MEMO_PAD, SCALING_FACTOR_MEMO_PAD,
					SCALING_FACTOR_MEMO_PAD));
			memoPad.setTransformByPositionIndex(iMemoIndex, transform);
	...
	public void renderMemoPad(final GL gl, final AGLEventListener containingView)
	{
		...
		gl.glPushName(generalManager.getViewGLCanvasManager().getPickingManager()
				.getPickingID(containingView.getID(), EPickingType.MEMO_PAD_SELECTION,
						MEMO_PAD_PICKING_ID));
		...
		textRenderer.draw3D("MEMO PAD", 1.3f, -2.2f, 0.01f, 0.005f); // scale
		// factor
		...
		while (iterMemoPathwayIndex.hasNext())
		{
			...
			gLPathwayTextureManager.renderPathway(gl, containingView, iPathwayId, 1f, false);
