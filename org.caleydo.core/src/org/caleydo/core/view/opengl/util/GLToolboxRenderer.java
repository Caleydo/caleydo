...
public class GLToolboxRenderer
{
			...
			final GeneralRenderStyle renderStyle)
	{
		...
		this(gl, generalManager, iContainingViewID, vecLeftPoint, bRenderLeftToRight,
				renderStyle);
	...
	public void render(final GL gl)
	{
		...
		if (layer != null)
		{
			// addIcon(gl, iRemoteViewID,
			// EPickingType.BUCKET_MOVE_HIERARCHY_UP_ICON_SELECTION,
			// iContainingViewID, EIconTextures.ARROW_LEFT);
			// addIcon(gl, iRemoteViewID,
			// EPickingType.BUCKET_REMOVE_ICON_SELECTION,
			// iContainingViewID, EIconTextures.ARROW_LEFT);
			// addIcon(gl, iRemoteViewID,
			// EPickingType.BUCKET_SWITCH_ICON_SELECTION,
			// iContainingViewID, EIconTextures.ARROW_LEFT);
		}
	...
	protected void addIcon(final GL gl, int iContainingViewID, EPickingType ePickingType,
			int iIconID, EIconTextures eIconTexture)
	{
		...
		if (bRenderLeftToRight)
		{
			...
			gl
					.glVertex3f(fRenderLenght + vecLeftPoint.x(), vecLeftPoint.y(),
							vecLeftPoint.z());
			gl.glTexCoord2f(texCoords.right(), texCoords.bottom());
			gl.glVertex3f(fRenderLenght + vecLeftPoint.x() + renderStyle.getButtonWidht(),
					vecLeftPoint.y(), vecLeftPoint.z());
			gl.glTexCoord2f(texCoords.right(), texCoords.top());
			gl.glVertex3f(fRenderLenght + vecLeftPoint.x() + renderStyle.getButtonWidht(),
					vecLeftPoint.y() + renderStyle.getButtonWidht(), vecLeftPoint.z());
			gl.glTexCoord2f(texCoords.left(), texCoords.top());
			gl.glVertex3f(fRenderLenght + vecLeftPoint.x(), vecLeftPoint.y()
					+ renderStyle.getButtonWidht(), vecLeftPoint.z());
		}
		else
		{
			gl
					.glVertex3f(vecLeftPoint.x(), fRenderLenght + vecLeftPoint.y(),
							vecLeftPoint.z());
			gl.glVertex3f(vecLeftPoint.x() + renderStyle.getButtonWidht(), fRenderLenght
					+ vecLeftPoint.y(), vecLeftPoint.z());
			gl.glVertex3f(vecLeftPoint.x() + renderStyle.getButtonWidht(), fRenderLenght
					+ vecLeftPoint.y() + renderStyle.getButtonWidht(), vecLeftPoint.z());
			gl.glVertex3f(vecLeftPoint.x(), fRenderLenght + vecLeftPoint.y()
					+ renderStyle.getButtonWidht(), vecLeftPoint.z());
		...
		gl.glPopName();
		tempTexture.disable();
		fRenderLenght = fRenderLenght + renderStyle.getButtonWidht()
	...
	// //camPos = view * camPos;
	//
	// camPos.setX(matrix[0] * camPos.x() + matrix[4] * camPos.y() + matrix[8] *
	// camPos.z() + matrix[12]);
	// camPos.setY(matrix[1] * camPos.x() + matrix[5] * camPos.y() + matrix[9] *
	// camPos.z() + matrix[13]);
	// camPos.setZ(matrix[2] * camPos.x() + matrix[6] * camPos.y() + matrix[10]
	// * camPos.z() + matrix[14]);
	// }
