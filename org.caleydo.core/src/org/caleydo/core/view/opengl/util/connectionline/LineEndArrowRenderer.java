...
public class LineEndArrowRenderer extends ALineEndRenderer {
	...
	public void render(GL2 gl, List<Vec3f> linePoints) {
		Vec3f arrowHead = isLineEnd1 ? linePoints.get(0) : linePoints.get(linePoints.size()-1);
		Vec3f prevArrowHeadPoint = isLineEnd1 ? linePoints.get(1) : linePoints.get(linePoints.size()-2);
		Vec2f direction = new Vec2f(arrowHead.x() - prevArrowHeadPoint.x(), arrowHead.y() - prevArrowHeadPoint.y());
