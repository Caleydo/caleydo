...
/**
 * Saves the project-state periodically.
 * Should only be added as multiple-execution object to a {@link DisplayLoopExecution} 
 * as usually managed by {@link IViewManager}.
 * @author Werner Puff
 */
public class AutoSaver
	...
	public void run() {
		if ((new Date()).getTime() > lastSaveTimeStamp.getTime() + DEFAULT_INTERVAL) {
			...
			Date stop = new Date();
			log.log(new Status(Status.INFO, GeneralManager.PLUGIN_ID, "AutoSaver: auto save took " + (stop.getTime() - start.getTime()) + " ms"));
	...
	/**
	 * Gets the timestamp when the most recent auto save operation has been performered.
	 * If no auto save operation has been performed yet, the initialization time
