...
/**
 * Helper class that chooses the data configuration based on parameters where
 * possible, or opens a dialog when it's unclear.
 * 
...
public class DataConfigurationChooser {
	...
	/**
	 * <p>
	 * Chooses a {@link IDataDomain}, a {@link RecordPerspective} and a
	 * {@link DimensionPerspective} based on the list of dataDomains specified.
	 * If multiple matches are available a dialog to make the user choose is
	 * opened.
	 * </p>
	 * 
	 * @param a list of possible data domains
	...
	public static DataConfiguration determineDataConfiguration(
		...
		if (possibleDataDomains.size() == 1) {
			...
			if (chosenDataDomain instanceof ATableBasedDataDomain) {
				...
				Collection<String> recordPerspectiveCandidates = DataConfigurationChooser
						.getRecordPerspectiveCandidates(tDataDomain.getTable(), false);
				if (recordPerspectiveCandidates.size() == 1) {
					recordPerspectiveID = recordPerspectiveCandidates.iterator().next();
					recordPerspective = tDataDomain.getTable().getRecordPerspective(
							recordPerspectiveID);
				}
				...
				if (dimensionPerspectiveCandidates.size() == 1) {
					...
					dimensionPerspective = tDataDomain.getTable().getDimensionPerspective(
	...
	/**
	 * <p>
	 * Checks if there is only one {@link RecordPerspective} from those
	 * registered with the {@link DataTable} a possible match considering the
	 ...
	 * </p>
	 * 
	 * @param table the data table from which to choose the available
	 *            perspectives
	 * @param considerOnlyPublic whether only public perspectives shall be used
	 *            (true) or whether to also include private perspectives
	 * @return the chosen perspectiveID if a unique ID could be identified, or
	 *         null
	 */
	...
	/**
	 * Same as {@link #getRecordPerspectiveCandidates(DataTable, boolean)} for
	 * dimensions.
	 * 
	 * @param table
	...
	/**
	 * Generic method for
	 * {@link #getRecordPerspectiveCandidates(DataTable, boolean)} and
	 ...
	 * @return
	 */
	private static ArrayList<String> getPerspectiveCandidates(DataTable table,
			boolean considerOnlyPublic, boolean isRecord) {
