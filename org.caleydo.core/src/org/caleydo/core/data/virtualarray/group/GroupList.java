...
// implements IGroupList<ConcreteType, VA, VADelta> {
{
	...
	public ArrayList<Float> determineRepresentativeElement(DataTable table,
			RecordVirtualArray recordVA, DimensionVirtualArray dimensionVA, int iGroupNr,
			boolean bGeneGroup) {

		ArrayList<Float> representative = new ArrayList<Float>();

		int iNrElementsInGroup = groups.get(iGroupNr).getSize();
		int iOffset = 0;
		...
		if (bGeneGroup) {
			for (Integer iDimensionIndex : dimensionVA) {
				...
				for (int index = 0; index < iNrElementsInGroup; index++) {
					fArExpressionValues[index] += table.getFloat(
							DataRepresentation.NORMALIZED, recordVA.get(iOffset + index),
							iDimensionIndex);
				}
