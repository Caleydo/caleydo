...
/**
 * A VADeltaItem contains all information for the modification of a single item in a virtual array. Typically several
 * items are held in a collection, a delta (see {@link VirtualArrayDelta}) Several types of operations are defined in
 * {@link EVAOperation} The item is implemented using static factories thereby guaranteeing that only valid parameters
 * are contained.
 *
 * @author Alexander Lex
 */

public class VADeltaItem implements IDeltaItem {
	...
	/**
	 * Static factory for a new delta item that removes an element at a specific position of a virtual array Take good
	 * care to remove items in the correct order, from back to front. Otherwise this can corrupt your data.
	 *
	 * @param index
	...
	/**
	 * Static Factory for new delta item that moves the element at the specified src index to the specified target
	 * index. See {@link IVirtualArray#move(int, int)} for further details.
	 *
	 * @param srcIndex
	 *            the src index
	 * @param targetIndex
	...
	/**
	 * Static Factory for new delta item that copies the element at the specified index and adds the new element at
	 * iIndex + 1 {@link IVirtualArray#copy(int)} for further details.
	 *
	 * @param index
	...
	/**
	 * Create a new VADeltaItem with properties specified in operation. The new item may take only one parameter,
	 * therefore {@link EVAOperation#ADD} and {@link EVAOperation#MOVE} can not be passed as an argument here
	 *
	 * @param operation
	...
	public VADeltaItem clone() {
		try {
			...
			throw new IllegalStateException("Something went wrong with the cloning, caught CloneNotSupportedException");
		}
	}
