...
implements INumericalCContainer 
{
	 ...
	 * Constructor
	 * 
	 * Pass a float array. The length of the array can not be modified after initialization
	 * 
	 ...
	 * 
	 * values that are NaN in the input are kept to be NaN
	 * 
	...
	private ICContainer normalize(final float fMin, final float fMax)
	{
		...
		{			
			...
			{	
				...
				{		
					fArTmpTarget[iCount] = (fArContainer[iCount] - fMin) / 
										(fMax - fMin);
				}
			}			
		}
	...
	/**
	 * Calculates the min and max of the container and sets them to the fMin and fMax class
	 * variables
	 */
