...
import org.jgraph.pad.coreframework.jgraphsubclassers.GPTransferable;
...
public class GPLibraryPanel extends JComponent {
	...
	private GPGraphpad graphpad;

	public boolean fullLibraryAccess = true;

	public GPLibraryPanel(String name, String[] files, int entrySize, GPGraphpad pad) {
		MAX = entrySize;
		graphpad = pad;
	...
	public void showPopupMenu(Component c, Point p) {
		...
		if (pad != null) {
			JPopupMenu pop = GPBarFactory.getInstance().createLibraryPopupMenu(graphpad.getCurrentDocument());
			pop.show(c, p.x, p.y);
		}
	}

	public ScrollablePanel addLibrary(String name) {
		ScrollablePanel tmp = new ScrollablePanel(name, graphpad);
		combo.addItem(tmp);
	...
	public class ScrollablePanel extends JPanel implements Scrollable {
		...
		public ScrollablePanel(String name, GPGraphpad pad) {
			super(new FixedWidthFlowLayout());
			graph = (JGraph) GPPluginInvoker.instanciateObjectForKey("JGraph.class");
            graph.getGraphLayoutCache()
            .setFactory((CellViewFactory) GPPluginInvoker.instanciateObjectForKey("ViewFactory.class"));
