...
public class FileExportJGX extends AbstractActionFile {
	...
	public void actionPerformed(ActionEvent e) {
		try {
			NamedOutputStream out = GPPluginInvoker.provideOutputStream(".jgx", getCurrentDocument().getDocComponent().getName(), false);
			getCurrentDocument().getDocComponent().setName(out.getName());
			JGraphJGX.getInstance(graphpad).write(out.getOutputStream(), getCurrentDocument());
		} catch (Exception ex) {
			ex.printStackTrace();
		}
	}
