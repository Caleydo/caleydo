...
public class ForceDirectedGraphLayout {
	...
	protected boolean isDataAvailable() {
		...
		if (graph.getNumberOfNodes() == 0) return false;
	...
	private double getDistance(Object node1, Object node2) {
		...
		if (distanceMatrix == null) return Double.POSITIVE_INFINITY;
		
		Map<Object, Double> map = distanceMatrix.get(node1);
		if (map == null) return Double.POSITIVE_INFINITY;
		
		Double distance = map.get(node2);
		
		if (distance == null) return Double.POSITIVE_INFINITY;
		return distance;
	}
	...
	protected void calculateDelta(Object node1, Object node2, double[] eqs) {
		...
		if (divisor7_8 == 0.0) divisor7_8 = 1.0;
				
		double divisor13_16 = Math.pow(divisor, 1.5);
		if (divisor13_16 == 0.0) divisor7_8 = 1.0;
	...
	private boolean forcesWithoutError() {
		for (Object node : nodesToLayout) {
			...
			if (Double.isNaN(force.getX()) || Double.isNaN(force.getY())) return false;
		}
	...
	public void layout(Rectangle2D area) {
		...
		L = scalingFactor * Math.max(10, Math.min(area.getWidth(), area.getHeight())) / diameter;
	...
	private boolean isPositionAvailable(Object node) {
		if (nodePositions == null) return false;
		
		Point2D pos = nodePositions.get(node);
		if (pos == null) return false;
		
		if (Double.isNaN(pos.getX()) || Double.isNaN(pos.getY())) return false;
	...
	private void substractForceInfluence(Object influencer) {
		...
		for (Object node : graph.getNodes()) {
			if (node != influencer) {
				...
				force.setLocation(force.getX() - subtractForce.getX(), force.getY() - subtractForce.getY());
			}
		}
	}
	...
	private void addForceInfluence(Object influencer) {
		...
		for (Object node : graph.getNodes()) {
			if (node != influencer) {
				...
				force.setLocation(force.getX() + addDelta.getX(), force.getY() + addDelta.getY());
			}
		}
	}
...
//		}
		...
		for (Object node : nodesToLayout) {
			...
			centeredPositions.put(node, new Point2D.Double(
				(point.getX() - minX) * fx + offsetX,
				(point.getY() - minY) * fy + offsetY
			));
		}
	}
	...
	public Point2D getNodePosition(Object node, boolean centered) {
		if (centered) {
			if (centeredPositions == null) return null;
