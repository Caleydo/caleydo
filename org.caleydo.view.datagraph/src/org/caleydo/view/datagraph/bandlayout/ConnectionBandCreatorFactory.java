...
public class ConnectionBandCreatorFactory {
	...
	public static AConnectionBandCreator getConnectionBandCreator(
			IDataGraphNode node1, IDataGraphNode node2, PixelGLConverter pixelGLConverter) {
...
//		boolean isOffsetHorizontal = false;
		...
		if (deltaX < 0) {
			if (deltaY < 0) {
				...
				return new LeftDownRightUpNodeRelationBandCreator(node1, node2, pixelGLConverter);
//				
...
//				}
				...
				//1-
				//-2
				return new LeftUpRightDownNodeRelationBandCreator(node1, node2, pixelGLConverter);
//				float spacingX = (float) ((position2.getX() - node2.getWidth() / 2.0f) - (position1
//						.getX() + node1.getWidth() / 2.0f));
...
//				}
			...
			if (deltaY < 0) {
				//2-
				//-1
				return new LeftUpRightDownNodeRelationBandCreator(node2, node1, pixelGLConverter);
...
//				}
				...
				//-1
				//2-
				return new LeftDownRightUpNodeRelationBandCreator(node2, node1, pixelGLConverter);
//				float spacingX = (float) ((position1.getX() - node1.getWidth() / 2.0f) - (position2
//						.getX() + node2.getWidth() / 2.0f));
//				float spacingY = (float) ((position1.getY() - node1.getHeight() / 2.0f) - (position2
//						.getY() + node2.getHeight() / 2.0f));
