...
public abstract class ANodeConnector {
	...
	public ANodeConnector(IDataGraphNode node, PixelGLConverter pixelGLconverter,
	...
	protected Point2D calcPointOnLineWithFixedX(Point2D pointOnLine, float vecX,
			float vecY, float fixedX, float minY, float maxY,
		...
		float pointY = (float) pointOnLine.getY() - ((float) pointOnLine.getX() - fixedX)
				* lambda;
	...
	protected Point2D calcPointOnLineWithFixedY(Point2D pointOnLine, float vecX,
			float vecY, float fixedY, float minX, float maxX,
		...
		float pointX = (float) pointOnLine.getX()
				- (lambda == 0 ? 0 : ((float) pointOnLine.getY() - fixedY) / lambda);
	...
	protected void calcBandAnchorPoints(boolean isEnd1, List<Vec3f> bandPoints) {
		if (isEnd1) {
			bandAnchorPoint1 = new Point2D.Float(bandPoints.get(0).x(), bandPoints.get(0)
					.y());
			bandAnchorPoint2 = new Point2D.Float(bandPoints.get(bandPoints.size() - 1)
					.x(), bandPoints.get(bandPoints.size() - 1).y());
		} else {

			bandAnchorPoint1 = new Point2D.Float(bandPoints
					.get(bandPoints.size() / 2 - 1).x(), bandPoints.get(
					bandPoints.size() / 2 - 1).y());
			bandAnchorPoint2 = new Point2D.Float(bandPoints.get(bandPoints.size() / 2)
					.x(), bandPoints.get(bandPoints.size() / 2).y());
		}
