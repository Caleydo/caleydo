 ...
 *
 */
public class CreateJaccardIndexScoreDialog extends TitleAreaDialog {
	...
	public void create() {
		super.create();
		this.setTitle("Create a new Jaccard Index Score");
	...
	private void save() {
		...
		if (group == null || group == ALL_GROUP) { // score all
			...
			for (Group g : strat.getRecordPerspective().getVirtualArray().getGroupList()) {
				composite.add(manager.addIfAbsent(new JaccardIndexScore(strat, g)));
			}
			s = composite;
		} else { // score single
			s = Scores.get().addIfAbsent(new JaccardIndexScore(label, strat, group));
		}
