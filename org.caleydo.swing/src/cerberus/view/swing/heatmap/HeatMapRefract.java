...
package cerberus.view.swing.heatmap;
...
public class HeatMapRefract extends Demo {
	
  public static void main(String[] args) {
    GLCanvas canvas = new GLCanvas();
    final Animator animator = new Animator(canvas);
    
    HeatMapRefract demo = new HeatMapRefract(canvas,animator);
...
//	    frame.setJMenuBar( menuBar );	  
  ...
  public void displayHeatmap(GLAutoDrawable drawable) {
    if (!initComplete) {
      return;
    }
...
//    gl.glPushMatrix();
    ...
    if ( this.refSet != null ) {
    	...
    	int iDimensionX = refSet.getDimensionSize(0);
    	...
    	final float fIncX = (fMaxX - fMinX) / (float) iDimX;
    	final float fIncY = (fMaxY - fMinY) / (float) iDimY;
    	...
    	if ( i_dataValues != null ) {
    		...
    		int iHistogram_Min = i_dataValues[0];
        	int iHistogram_Max = i_dataValues[0];
        	
	    	for ( int i=0; i < iDimensionX; i++ ) {
			    gl.glBegin( GL.GL_TRIANGLE_FAN );
			    			  
			    if (  iHistogram_Min > i_dataValues[i]) {
			    	iHistogram_Min = i_dataValues[i];
			    } else if ( iHistogram_Max < i_dataValues[i]) {
			    	iHistogram_Max = i_dataValues[i];
			    }
			    
			    float fValue = (float) i_dataValues[i] / 100.0f;
			    
			    gl.glColor3f( fValue,0,0 );
			    
					//gl.glColor3f( 1,0,0 );
					gl.glVertex3f( fNowX, fNowY, 0.0f );
					//gl.glColor3f( 1,0,1 );
					gl.glVertex3f( fNextX, fNextY, 0.0f );
					//gl.glColor3f( 0,1,0 );
					gl.glVertex3f( fNowX, fNextY, 0.0f );
	    	...
	    	System.out.println("min= " + 
	    			Integer.toString(iHistogram_Min) + 
	    			" max=" + Integer.toString(iHistogram_Max) );
  ...
  public int[] createHistogram(final int iHistogramLevels) {
	  ...
	  if (( i_dataValues == null )&&( i_dataValues.length > 0)) {
		  return null;
	  }
	  
	 
	  int iOffset = refBufferSelection.getOffset();
	  int iUpperBorder = refBufferSelection.length()+ refBufferSelection.getOffset();
	  ...
	  if ( i_dataValues.length < 
		  ...
		  refBufferSelection.setLength( 
				  i_dataValues.length - refBufferSelection.getOffset() );
	  }
	  
	  int iMaxValue = i_dataValues[iOffset];
	  int iMinValue = i_dataValues[iOffset];
	
	  
	  /*  search mix - max values ... */
	  for ( int i = iOffset+1; i < iUpperBorder; i++ ) {
		  
		  if ( i_dataValues[i] < iMinValue) {
			  iMinValue = i_dataValues[i];
		  }
		  else if ( i_dataValues[i] > iMaxValue) {
			  iMaxValue = i_dataValues[i];
		  }
	  }
	  
	  int[] iHistogramCountResults = new int[iHistogramLevels];	  
	  int[] iHistogramRanges = new int[iHistogramLevels];
	  
	  float  fRangeInc = (float)(iMaxValue - iMinValue) / 
	  	(float)(iHistogramLevels);
	  float fRangeBorder = fRangeInc;
	  
	  /* calculate histogram intervall 
	   * ranges with equal spacing...*/
	  for ( int i=0; i< iHistogramLevels;i++ ) {
		  iHistogramRanges[i] = (int) fRangeBorder;
		  fRangeBorder += fRangeInc;
	  }
	  
	  int iHistogramLevelReduced = iHistogramLevels;	  
	  int iMaxCountAllIntervalls = 0;	  
	  boolean bSearchIntervall = true;
	  
	  
	  for ( int i = iOffset; i < iUpperBorder; i++) {
		  
		  int j = 0;
		  bSearchIntervall = true;
		  
		  while (( bSearchIntervall )&&( j < iHistogramLevelReduced )) {
			  
//			  int iBuffer = iHistogramRanges[j];
//			  int iTest = i_dataValues[i];
			  
			  if ( i_dataValues[i] < iHistogramRanges[j] ) {
				  iHistogramCountResults[j]++;
				  
				  if ( iHistogramCountResults[j] > iMaxCountAllIntervalls ) {
					  iMaxCountAllIntervalls = iHistogramCountResults[j];
				  }
	  ...
	  //TODO: creat class for histogram data and avoid global variable "iHistrogemMaxCountAllIntervalls"
	  iHistrogemMaxCountAllIntervalls = iMaxCountAllIntervalls;
		  
	  iHistrogemMaxValue = iMaxValue;
	  iHistrogemMinValue = iMinValue;
	  
	  System.out.println("  maxCount=" + Integer.toString(iMaxCountAllIntervalls) );
	  
	  return iHistogramCountResults;
  }
...
//		    		(float) iHistogramSpacing;
//		    	float fIncY = (fMaxY - fMinY) / 
//		    		(float) iHistrogemMaxCountAllIntervalls;
//		    	
//		    	float fNowX = fMinX;
//		    	float fNextX = fNowX + fIncX;
//		    	
...
//	        	
//		    	for ( int i=0; i < iHistogramSpacing; i++ ) {
//				    gl.glBegin( GL.GL_TRIANGLE_FAN );
//				    			  
//				    if ( bToggleColor) {
...
//				    }
//				    
//				    float fBar =  fMinY + fIncY * iHistogramIntervalls[i];
//				    
...
//	  }
  	...
  	public void displayHeatmapSimple(GLAutoDrawable drawable) {
	    if (!initComplete) {
	      return;
	    }
...
//	    gl.glEnable(GL.GL_DEPTH_TEST);
	    ...
	    if ( this.refSet != null ) {
	    	...
	    	if ( i_dataValues != null ) {
		    	...
		    	if ( refSet.hasCacheChanged(iSetCacheId) ) {
	    			...
	    			if ( select != null ) {
	    				System.out.println(" len=" + 
	    						Integer.toString( select.length() ) );
	    			}
	    		}
		    	...
		    	float fRange = (float) (iHistrogemMaxValue - iHistrogemMinValue);
		    	
		    	int iBuffer_Length = iHeatmapX*iHeatmapY;
		    	
		    	if ( iBuffer_Length  > i_dataValues.length) { 
		    		iBuffer_Length = i_dataValues.length;
		    	}
		    	
		    	int iDataValues_Langth = iHeatmapX*iHeatmapY;
		    	
		    	if ( iDataValues_Langth > i_dataValues.length ) {
		    		iDataValues_Langth = i_dataValues.length;
		    	}
		    	
		    	for ( int i=0; i < iDataValues_Langth; i++ ) {
				    gl.glBegin( GL.GL_TRIANGLE_FAN );
				    			  
				    float fValue = (i_dataValues[i] - iHistrogemMinValue)/fRange;
				    
				    gl.glColor3f( fValue , 1.0f-fValue ,0 );
...
//					    gl.glVertex3f( fNowX, fNowY, fValue );
//						gl.glVertex3f( fNextX, fNowY, fValue );
//						gl.glVertex3f( fNextX, fNextY, fValue );
//						gl.glVertex3f( fNowX, fNextY, fValue );
						
						gl.glVertex3f( fNowX, fNowY, 0.0f );
						gl.glVertex3f( fNextX, fNowY, 0.0f );
						gl.glVertex3f( fNextX, fNextY, 0.0f );
						gl.glVertex3f( fNowX, fNextY, 0.0f );												
					gl.glEnd();
