...
public class MappedDataRenderer {
	 ...
	 * 
	 * @param width the width of the drawing space of this renderer
	 * @param height the height of the drawing space of this renderer
	 * @param xOffset how much this renderer is translated in x direction from the window origin
	 * @param yOffset how much this renderer is translated in y direction from the window origin
	 * @param rowHeight the height of a single row in the renderer rows
	 */
	...
	private void prepareData(TablePerspective tablePerspective, ArrayList<ElementLayout> rowLayouts,
		...
		for (int rowCount = 0; rowCount < davidIDs.size(); rowCount++) {
			...
			if (dataDomain.getLabel().toLowerCase().contains("copy")) {
				...
				tablePerspectiveLayout.setRenderer(new MutationStatusRowContentRenderer(geneID, davidID, dataDomain,
						tablePerspective, experimentPerspective, parentView, this, group, isHighlightLayout));
			}
			else {
