	...
	implements GLEventListener, MouseListener, MouseMotionListener
{
	...
	public GearsMain()
	{
		gearsMouseListener = new GearsMouse(this);
	}

	public void runMain()
	{

		GearsMain refmyGrears = new GearsMain();

		// gearsMouseListener = new GearsMouse(refmyGrears);
	...
	public synchronized void setViewAngles(float fView_RotX, float fView_RotY, float fViewRotZ)
	{
	...
	public void init(GLAutoDrawable drawable)
	{
		...
		// drawable.addMouseListener( gearsMouseListener );
		// drawable.addMouseMotionListener( gearsMouseListener );
	...
	private void gear(GL gl, float inner_radius, float outer_radius, float width, int teeth,
			float tooth_depth)
	{
		int i;
		float r0, r1, r2;
		...
		for (i = 0; i <= teeth; i++)
		{
			...
			gl.glVertex3f(r0 * (float) Math.cos(angle), r0 * (float) Math.sin(angle),
					width * 0.5f);
			gl.glVertex3f(r1 * (float) Math.cos(angle), r1 * (float) Math.sin(angle),
					width * 0.5f);
			if (i < teeth)
			{
				gl.glVertex3f(r0 * (float) Math.cos(angle), r0 * (float) Math.sin(angle),
	...
	{
		gearsMouseListener.mousePressed(e);
	}

	public void mouseReleased(MouseEvent e)
	{
		gearsMouseListener.mouseReleased(e);
	}
	...
	{
		gearsMouseListener.mouseDragged(e);
	}
