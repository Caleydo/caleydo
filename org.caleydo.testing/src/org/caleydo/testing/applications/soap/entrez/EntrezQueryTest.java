...
public class EntrezQueryTest
{
	...
	public static void main(String[] args) throws Exception
	{
		...
		try
		{
			...
			// +": ");
			// System.out.println(res.getDocSum()[i].getItem()[k].get_any()[0].
			// getValue());
			// }
			// }
			...
			for (int iGeneCount = 0; iGeneCount < res.getEntrezgeneSet().getEntrezgene().length; iGeneCount++)
			{
				sAccession = res.getEntrezgeneSet().getEntrezgene(0).getEntrezgene_comments()
						...
						.getGeneCommentary_products().getGeneCommentary(0)
						.getGeneCommentary_accession();

				System.out.println("Accession number for gene with GeneId " + iGeneID
						+ " is: " + sAccession);
			}
		}
		catch (Exception e)
